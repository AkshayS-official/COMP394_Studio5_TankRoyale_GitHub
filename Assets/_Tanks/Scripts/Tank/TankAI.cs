...
namespace _Tanks.Scripts.Tank
{
    public class TankAI : MonoBehaviour
    {
        ...
        public BehaviorType behavior = BehaviorType.Brute;
        // ----------------------------------------
        ...
        private bool fastRetreating = false;
        private float fastRetreatTimer = 0f;
        
        private float bruteCooldown = 0f;
        ...
        void Update()
        {
            ...
            if (behavior == BehaviorType.Fast && fastRetreating)
            {
                fastRetreatTimer -= Time.deltaTime;
                if (fastRetreatTimer <= 0)
                    fastRetreating = false;
            }
            
            if (behavior == BehaviorType.Brute && bruteCooldown > 0)
                bruteCooldown -= Time.deltaTime;
        ...
        void SeekUpdate()
        {
            ...
            if (m_CurrentTarget != null)
            {
                ...
                if (targetDistance < 3.0f)
                {
                    ...
                    if (behavior != BehaviorType.Brute)
                    {
                    ...
                    if (targetDistance < m_MaxShootingDistance)
                    {
                        if (!NavMesh.Raycast(transform.position, m_CurrentTarget.position, out var hit, ~0))
                        {
                            ...
                            if (m_ShotCooldown <= 0.0f)
                            {
                                ...
                                // NOT SURE OF THIS
                                if (behavior == BehaviorType.Fast)
                                {
                                    fastRetreating = true;
                                    fastRetreatTimer = 1.5f;   // retreat duration
                                }
                            }
                        }
                    }
                ...
                // ------------------------------------------
                // ADDED: BEHAVIOR-BASED MOVEMENT CONTROL
                // ------------------------------------------
                ...
                if (behavior == BehaviorType.Sniper)
                {
                    float safeMin = m_MaxShootingDistance * 0.75f;   
                    float safeMax = m_MaxShootingDistance * 0.95f;

                    if (targetDistance < safeMin)
                    {
                        StartFleeing();
                        return;
                    }

                    if (targetDistance > safeMax)
                        m_IsMoving = true;
                    else
                        m_IsMoving = false;
                }
        ...
        private void FixedUpdate()
        {
            ...
            float speed = m_Movement.m_Speed;

            if (behavior == BehaviorType.Brute && m_Shooting.IsCharging)
                speed *= 0.5f;

            float moveAmount = Mathf.Clamp01(orientDot) * speed * Time.deltaTime;
